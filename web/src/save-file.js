...
export async function saveFile(file, {read, exportFile}, progress = () => {}, host = globalThis) {
  ...
  if (typeof host.showSaveFilePicker === 'function') {
    ...
    try { await copyFile(file, read, writer, progress); await writer.close(); }
    ...
    downloadFile(source, name, host);
    return 'download_started';
  }
}

export async function copyFile(file, read, writer, progress = () => {}, signal) {
...
export function downloadFile(source, name, host = globalThis) {
  // A normal download has no observable completion. Retain its URL for the
  // document lifetime so slow downloads can finish reading the disk-backed File.
