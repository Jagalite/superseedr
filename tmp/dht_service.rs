...
const INTERNAL_DHT_LOOKUP_MAX_VISITS_PER_FAMILY: usize = 320;
const INTERNAL_DHT_BACKGROUND_MAX_VISITS_PER_FAMILY: usize = 240;
...
const INTERNAL_DHT_ROUTE_WARM_LIMIT: usize = 12;
const INTERNAL_DHT_ROUTE_WARM_MAX_VISITS: usize = 72;
...
fn dht_family_label(is_ipv6: bool) -> &'static str {
    if is_ipv6 {
        "ipv6"
    } else {
        "ipv4"
...
impl InternalPrototypeClient {
    ...
    async fn bind_with_recovery(
        ...
        client.recover_unhealthy_routes().await;
    ...
    async fn query_family_get_peers(
        ...
        let (active_routes_available, fast_frontier_available) = {
            ...
            (
                ...
                fast_frontier.recent_count,
            )
        };
        let cached_routes_available = self.discovered_nodes.lock().await.family_count(is_ipv6);
        let family_max_visits = family_max_frontier_candidates(is_ipv6, purpose);
        ...
        let _ = self.spawn_family_get_peers_round(
            ...
            &mut join_set,
            &mut metrics,
            initial_wave_limit.min(family_max_visits),
        ...
        let ended_reason = if visited.len() >= family_max_visits {
            "visit_cap_reached"
    ...
    fn spawn_family_get_peers_round(
        ...
        let available_slots = max_concurrent_queries
            ...
            .min(spawn_limit);
        if visited.len() >= max_visits || available_slots == 0 {
            return 0;
        }
        ...
        for candidate in to_visit {
            ...
            let source = candidate.source;
            self.spawn_family_get_peers_query(socket, info_hash, candidate, join_set, metrics);
            spawned += 1;
            match source {
    ...
    async fn handle_family_get_peers_response(
        ...
        let (accepted_nodes, rejected_nodes) = frontier.insert_discovered_nodes(
            next_nodes,
            visited,
            family_max_frontier_candidates(is_ipv6, metrics.purpose),
        );
    ...
    async fn seed_family_nodes(
        ...
        let prefer_fast_frontier_only = family_policy
        ...
        let frontier_nodes = prioritize_non_bootstrap_nodes(frontier_snapshot.nodes, bootstrap_nodes);
        ...
        let route_nodes = if prefer_fast_frontier_only {
            let mut nodes = frontier_nodes;
            nodes.extend(
                active_nodes
                    .into_iter()
            ...
            let mut nodes = active_nodes;
            nodes.extend(
                cached_nodes
                    .into_iter()
                .filter(|addr| !active_node_addrs.contains(addr)),
            );
            nodes
        };
        ...
        if internal_probe_enabled() && target.is_some() {
            ...
            emit_internal_probe(format!(
                "event=query_seed_state family={} target={} active_total={} active_with_node_id={} active_positive_routes={} active_fast_eligible={} active_lookup_proven={} active_lookup_fast_eligible={} fast_frontier_available={} warm_mature_ipv4_lookup={} collapse_recovery_seed_mode={} active_seed_candidates={} cached_seed_candidates={} bootstrap_candidates={} ipv4_bucket_count={} ipv4_bucketed_routes={} ipv4_overflow_routes={} ipv4_max_bucket_len={} ipv4_bucket_summary={}",
                family_policy.label(),
                target_hex,
                ...
                prefer_fast_frontier_only,
                collapse_recovery_seed_mode,
        ...
        let bootstrap_front_limit = if collapse_recovery_seed_mode {
            ordered_bootstrap_nodes
                .len()
                .min(INTERNAL_DHT_INITIAL_QUERY_FANOUT)
        } else if !route_nodes.is_empty()
            && fast_frontier_ready_count < INTERNAL_DHT_INITIAL_QUERY_FANOUT
        {
            ordered_bootstrap_nodes
                .len()
                ...
                .min(
                    INTERNAL_DHT_INITIAL_QUERY_FANOUT
                        .saturating_sub(fast_frontier_ready_count),
                )
        } else {
            0
        };
        let front_cached_limit = if collapse_recovery_seed_mode {
            0
        } else {
            cached_limit
    ...
    fn record_internal_family_lookup_summary(
        ...
        summary.insert("family".into(), serde_json::json!(metrics.family));
        summary.insert("purpose".into(), serde_json::json!(metrics.purpose));
        summary.insert(
            "shared_lookup".into(),
            serde_json::json!(metrics.shared_lookup),
        );
        summary.insert(
            "bootstrap_nodes".into(),
            serde_json::json!(metrics.bootstrap_nodes),
        );
        summary.insert(
            "active_routes_available".into(),
            serde_json::json!(metrics.active_routes_available),
        );
        summary.insert(
            "fast_frontier_available".into(),
            serde_json::json!(metrics.fast_frontier_available),
        );
        summary.insert(
            "cached_routes_available".into(),
            serde_json::json!(metrics.cached_routes_available),
        );
        summary.insert(
            "seeded_total".into(),
            serde_json::json!(metrics.seeded_total),
        );
        summary.insert(
            "seeded_bootstrap".into(),
            serde_json::json!(metrics.seeded_bootstrap),
        );
        summary.insert(
            "seeded_cached".into(),
            serde_json::json!(metrics.seeded_cached),
        );
        summary.insert(
            "initial_wave_limit".into(),
            serde_json::json!(metrics.initial_wave_limit),
        );
        summary.insert(
            "queries_spawned".into(),
            serde_json::json!(metrics.queries_spawned),
        );
        summary.insert(
            "bootstrap_queries_spawned".into(),
            serde_json::json!(metrics.bootstrap_queries_spawned),
        );
        summary.insert(
            "cached_queries_spawned".into(),
            serde_json::json!(metrics.cached_queries_spawned),
        );
        summary.insert(
            "discovered_queries_spawned".into(),
            serde_json::json!(metrics.discovered_queries_spawned),
        );
        summary.insert(
            "query_successes".into(),
            serde_json::json!(metrics.query_successes),
        );
        summary.insert(
            "query_failures".into(),
            serde_json::json!(metrics.query_failures),
        );
        summary.insert(
            "responses_with_peers".into(),
            serde_json::json!(metrics.responses_with_peers),
        );
        summary.insert(
            "responses_with_peers_before_first_batch".into(),
            serde_json::json!(metrics.responses_with_peers_before_first_batch),
        );
        summary.insert(
            "responses_with_peers_after_first_batch".into(),
            serde_json::json!(metrics.responses_with_peers_after_first_batch),
        );
        summary.insert(
            "peer_values_seen".into(),
            serde_json::json!(metrics.peer_values_seen),
        );
        summary.insert(
            "peer_values_before_first_batch".into(),
            serde_json::json!(metrics.peer_values_before_first_batch),
        );
        summary.insert(
            "peer_values_after_first_batch".into(),
            serde_json::json!(metrics.peer_values_after_first_batch),
        );
        summary.insert(
            "duplicate_peers_filtered".into(),
            serde_json::json!(metrics.duplicate_peers_filtered),
        );
        summary.insert(
            "duplicate_peers_before_first_batch".into(),
            serde_json::json!(metrics.duplicate_peers_before_first_batch),
        );
        summary.insert(
            "duplicate_peers_after_first_batch".into(),
            serde_json::json!(metrics.duplicate_peers_after_first_batch),
        );
        summary.insert(
            "unique_peers_before_first_batch".into(),
            serde_json::json!(metrics.unique_peers_before_first_batch),
        );
        summary.insert(
            "unique_peers_after_first_batch".into(),
            serde_json::json!(metrics.unique_peers_after_first_batch),
        );
        summary.insert(
            "nodes_discovered".into(),
            serde_json::json!(metrics.nodes_discovered),
        );
        summary.insert(
            "nodes_accepted".into(),
            serde_json::json!(metrics.nodes_accepted),
        );
        summary.insert(
            "nodes_rejected".into(),
            serde_json::json!(metrics.nodes_rejected),
        );
        summary.insert(
            "batches_published".into(),
            serde_json::json!(metrics.batches_published),
        );
        summary.insert(
            "peers_published".into(),
            serde_json::json!(metrics.peers_published),
        );
        summary.insert("max_pending".into(), serde_json::json!(metrics.max_pending));
        summary.insert(
            "max_inflight".into(),
            serde_json::json!(metrics.max_inflight),
        );
        summary.insert(
            "first_value_source".into(),
            serde_json::json!(metrics.first_value_source),
        );
        summary.insert("visited".into(), serde_json::json!(visited));
        summary.insert("peers".into(), serde_json::json!(peers));
        summary.insert(
            "visit_cap_reached".into(),
            serde_json::json!(
                visited >= family_max_frontier_candidates(metrics.family == "ipv6", metrics.purpose)
            ),
        );
        summary.insert(
            "peer_cap_reached".into(),
            serde_json::json!(peers >= INTERNAL_DHT_MAX_RETURNED_PEERS),
        );
        summary.insert("ended_reason".into(), serde_json::json!(ended_reason));
        summary.insert(
            "elapsed_ms".into(),
            serde_json::json!(network_metrics::elapsed_ms(lookup_started_at)),
        );
    ...
    async fn mark_route_queried(&self, addr: SocketAddr) {
        let mut active_routes = self.active_routes.lock().await;
        let _ = active_routes.mark_queried(addr, StdInstant::now());
    }
    ...
    async fn record_route_query_success(&self, addr: SocketAddr, node_id: Option<[u8; 20]>) {
        let mut discovered_nodes = self.discovered_nodes.lock().await;
        let resolved_node_id = node_id.or_else(|| discovered_nodes.node_id_for(addr));
        discovered_nodes.record_success(addr, resolved_node_id);
        drop(discovered_nodes);
        let mut active_routes = self.active_routes.lock().await;
        active_routes.record_route_query_success(addr, resolved_node_id);
    }

    async fn record_route_refresh_success(
        &self,
        addr: SocketAddr,
        ...
        discovered_nodes.record_success(addr, resolved_node_id);
        let discovered_after = discovered_nodes.family_count(is_ipv6);
        drop(discovered_nodes);
        let mut active_routes = self.active_routes.lock().await;
        let active_before = active_routes.family_count(is_ipv6);
        let active_present_before = active_routes.contains(addr);
        let refill_floor = family_active_route_refill_floor(is_ipv6);
        let should_reinforce = true;
        active_routes.record_success(addr, resolved_node_id);
        let active_after = active_routes.family_count(is_ipv6);
        let active_present_after = active_routes.contains(addr);
        if internal_probe_enabled() {
            emit_internal_probe(format!(
                "event=route_refresh_success family={} context={} addr={} discovered_before={} discovered_after={} discovered_present_before={} active_before={} active_after={} active_present_before={} active_present_after={} refill_floor={} should_reinforce={} node_id_resolved={}",
                dht_family_label(is_ipv6),
                context,
                addr,
                discovered_before,
                discovered_after,
                discovered_present_before,
                active_before,
                active_after,
                active_present_before,
                active_present_after,
                refill_floor,
                should_reinforce,
                resolved_node_id.is_some(),
            ));
        }
    }
    ...
    async fn announce_family_peer(
        ...
        for token in tokens.into_iter().take(INTERNAL_DHT_TOKEN_TARGET_LIMIT) {
                ...
                .await
            {
                ...
                self.record_route_failure(token.addr, "announce_token")
                    .await;
                self.announce_tokens
                    .lock()
                    .await
...
struct InternalPrototypeDiscoveredNodes {
    ...
    next_recency_epoch: u64,
}

#[derive(Debug, Clone)]
struct InternalPrototypeActiveRoutes {
    ipv4: InternalPrototypeRouteTable,
    ipv6: InternalPrototypeRouteTable,
}

#[derive(Debug, Clone)]
struct InternalPrototypeRouteTable {
    is_ipv6: bool,
    local_node_id: Option<[u8; 20]>,
    buckets: BTreeMap<u8, VecDeque<InternalPrototypeNodeRecord>>,
    replacements: BTreeMap<u8, VecDeque<InternalPrototypeNodeRecord>>,
...
impl InternalPrototypeDiscoveredNodes {
    fn allocate_recency_epoch(&mut self) -> u64 {
        self.next_recency_epoch = self.next_recency_epoch.saturating_add(1);
        self.next_recency_epoch
    }
    ...
    fn insert(&mut self, node: InternalCompactNode) {
        let recency_epoch = self.allocate_recency_epoch();
        let now = StdInstant::now();
...
impl Default for InternalPrototypeActiveRoutes {
    fn default() -> Self {
        Self {
            ipv4: InternalPrototypeRouteTable::new(false),
            ipv6: InternalPrototypeRouteTable::new(true),
        }
    }
}

impl InternalPrototypeRouteTable {
    fn policy(&self) -> InternalFamilyPolicy {
        InternalFamilyPolicy::new(self.is_ipv6)
    }

    fn new(is_ipv6: bool) -> Self {
        Self {
            is_ipv6,
            local_node_id: None,
    ...
    fn all_records(&self) -> Vec<InternalPrototypeNodeRecord> {
        let mut records = self.snapshot_records();
        for bucket in self.replacements.values() {
            records.extend(bucket.iter().cloned());
        }
        records
    }
    ...
    fn total_count(&self) -> usize {
        self.overflow.len() + self.buckets.values().map(VecDeque::len).sum::<usize>()
    }
    ...
    fn snapshot_replacement_refresh_candidates(&self, target: Option<[u8; 20]>) -> Vec<SocketAddr> {
        ...
        for (bucket, replacements) in &self.replacements {
            let live_count = self.buckets.get(bucket).map_or(0, VecDeque::len);
            let bucket_underfilled = live_count < policy.route_bucket_size();
            let mut candidates = replacements.iter().cloned().collect::<Vec<_>>();
            candidates.sort_by(|left, right| {
                compare_refresh_due_records(left, right, self.is_ipv6, now)
                    .then_with(|| {
                        compare_active_route_records(left, right, target.as_ref(), self.is_ipv6, now)
                    })
            });
        ...
        preferred.into_iter().map(|record| record.addr).collect()
    }

    fn snapshot_refresh_due_candidates(
        &self,
        target: Option<[u8; 20]>,
        limit: usize,
    ) -> Vec<SocketAddr> {
        let now = StdInstant::now();
        let mut seen = HashSet::new();
        let mut candidates = self.snapshot_records();
        candidates.sort_by(|left, right| {
            compare_refresh_due_records(left, right, self.is_ipv6, now)
                .then_with(|| {
                    compare_active_route_records(left, right, target.as_ref(), self.is_ipv6, now)
        ...
        candidates
            .into_iter()
            .filter(|record| seen.insert(record.addr))
            .take(limit)
            .map(|record| record.addr)
            .collect()
    }

    fn mark_queried(&mut self, addr: SocketAddr, when: StdInstant) -> bool {
        for record in &mut self.overflow {
            if record.addr == addr {
                record.last_queried_at = Some(when);
                return true;
            }
        }
        ...
        for bucket in self.replacements.values_mut() {
            for record in bucket {
                if record.addr == addr {
                    record.last_queried_at = Some(when);
                    return true;
    ...
    fn insert_replacement_candidate(&mut self, candidate: InternalPrototypeNodeRecord) -> bool {
        let Some(local_node_id) = self.local_node_id else {
            return false;
        };
        let Some(bucket) = is_bucket_eligible_route(&candidate, self.is_ipv6)
            .then_some(())
            .and(
                candidate
                    .node_id
        ...
        let replacements = self.replacements.entry(bucket).or_default();
        replacements.retain(|existing| existing.addr != candidate.addr);
        replacements.push_back(candidate);
        self.sort_bucket_records(bucket, now);

        let promoted = self.promote_replacement_candidate(bucket, candidate_addr, now);
        let replacements = self.replacements.entry(bucket).or_default();
        let mut ordered = replacements.drain(..).collect::<Vec<_>>();
        ordered.sort_by(|left, right| {
            compare_active_route_retention_records_with_time(left, right, self.is_ipv6, now)
        });
        ordered.truncate(policy.route_bucket_size());
        *replacements = ordered.into();
        promoted
    }

    fn sort_bucket_records(&mut self, bucket: u8, now: StdInstant) {
        if let Some(live) = self.buckets.get_mut(&bucket) {
            let mut ordered = live.drain(..).collect::<Vec<_>>();
            ordered.sort_by(|left, right| {
                compare_active_route_retention_records_with_time(left, right, self.is_ipv6, now)
            });
            *live = ordered.into();
        }
        if let Some(replacements) = self.replacements.get_mut(&bucket) {
            let mut ordered = replacements.drain(..).collect::<Vec<_>>();
            ordered.sort_by(|left, right| {
                compare_active_route_retention_records_with_time(left, right, self.is_ipv6, now)
            });
    ...
    fn promote_replacement_candidate(
        &mut self,
        bucket: u8,
        candidate_addr: SocketAddr,
        now: StdInstant,
    ) -> bool {
        let family_policy = self.policy();
        let Some(replacements) = self.replacements.get_mut(&bucket) else {
            return false;
        };
        let Some(candidate_index) = replacements
            .iter()
            .position(|record| record.addr == candidate_addr)
        else {
            return false;
        };
        let candidate = replacements[candidate_index].clone();
        let candidate_is_vetted = candidate.peer_lookup_success_count > 0
            || candidate.lookup_success_count > 0
            || candidate.route_query_success_count > 0;
        if !candidate_is_vetted {
            return false;
        }

        let live = self.buckets.entry(bucket).or_default();
        if live.len() < family_policy.route_bucket_size() {
            let candidate = replacements.remove(candidate_index).expect("replacement candidate");
            live.push_back(candidate);
            self.sort_bucket_records(bucket, now);
            return true;
        }

        let Some(worst_live) = live.back().cloned() else {
            return false;
        };
        let should_promote = family_policy.is_stale_route_record(&worst_live, now)
            || compare_active_route_retention_records_with_time(
                &candidate,
                &worst_live,
                self.is_ipv6,
                now,
        ...
        let candidate = replacements.remove(candidate_index).expect("replacement candidate");
        let demoted = live.pop_back().expect("live route");
        live.push_back(candidate);
        replacements.push_back(demoted);
        self.sort_bucket_records(bucket, now);
        true
    }

    fn rebalance(&mut self) {
        self.replace_records(self.all_records());
    }
    ...
    fn replace_records(&mut self, records: Vec<InternalPrototypeNodeRecord>) {
        ...
        for record in records {
            if seen_addrs.insert(record.addr) {
                deduped.push(record);
            }
        }
        if self.local_node_id.is_none() || !family_policy.uses_bucketed_route_table() {
            let mut ordered = deduped;
            let now = StdInstant::now();
            ordered.sort_by(|left, right| {
                compare_active_route_retention_records_with_time(left, right, self.is_ipv6, now)
            });
            ordered.truncate(family_policy.active_route_limit());
            self.buckets.clear();
        ...
        let mut bucket_candidates = BTreeMap::<u8, Vec<InternalPrototypeNodeRecord>>::new();
        let mut overflow = Vec::new();

        for record in deduped {
            let bucket = self.local_node_id.and_then(|local_node_id| {
                is_bucket_eligible_route(&record, self.is_ipv6)
                    .then_some(())
                    .and(
                        record
                            .node_id
            ...
            if let Some(bucket) = bucket {
                bucket_candidates.entry(bucket).or_default().push(record);
            } else {
        ...
        let mut replacements = BTreeMap::<u8, VecDeque<InternalPrototypeNodeRecord>>::new();
        let mut all_buckets = bucket_candidates
            .keys()
            .copied()
            .collect::<HashSet<_>>()
            .into_iter()
        ...
        for bucket in all_buckets {
            let mut candidates = bucket_candidates.remove(&bucket).unwrap_or_default();
            candidates.sort_by(|left, right| {
                compare_active_route_retention_records_with_time(left, right, self.is_ipv6, now)
            });
            let live_limit = family_policy.route_bucket_size();
            let live_len = candidates.len().min(live_limit);
            let mut live = candidates.drain(..live_len).collect::<Vec<_>>();
            let mut replacement = candidates;
            replacement.truncate(live_limit);

            if !live.is_empty() {
                bucketed.insert(bucket, VecDeque::from(std::mem::take(&mut live)));
            }
            if !replacement.is_empty() {
                replacements.insert(bucket, VecDeque::from(replacement));
            }
        }
        ...
        overflow.truncate(family_policy.route_unbucketed_limit());
        let mut overflow = VecDeque::from(overflow);

        while bucketed.values().map(VecDeque::len).sum::<usize>() + overflow.len()
            > family_policy.active_route_limit()
        {
            if overflow.pop_back().is_some() {
                continue;
            }
            if !trim_bucketed_routes(&mut bucketed, self.is_ipv6, now) {
                break;
            }
        }
    ...
    fn probe_summary(&self) -> InternalIpv4RouteTableProbeSummary {
        ...
        let max_bucket_len = self.buckets.values().map(VecDeque::len).max().unwrap_or(0);
...
impl InternalPrototypeActiveRoutes {
    fn route_table(&self, is_ipv6: bool) -> &InternalPrototypeRouteTable {
        if is_ipv6 {
            &self.ipv6
        } else {
            &self.ipv4
    ...
    fn route_table_mut(&mut self, is_ipv6: bool) -> &mut InternalPrototypeRouteTable {
        if is_ipv6 {
            &mut self.ipv6
        } else {
            &mut self.ipv4
    ...
    fn set_local_node_id(&mut self, node_id: [u8; 20]) {
        ...
        self.ipv6.set_local_node_id(node_id);
    }
    ...
    fn family_probe_summary(&self, is_ipv6: bool) -> InternalActiveRouteProbeSummary {
        let route_table = self.route_table(is_ipv6);
        let family_nodes = route_table.snapshot_records();
        let now = StdInstant::now();
        let family_policy = InternalFamilyPolicy::new(is_ipv6);
        family_nodes.iter().fold(
            ...
            |mut summary, record| {
                ...
                if record.success_count > 0 && family_policy.is_live_route_record(record, now) {
    ...
    fn record_soft_failure(&mut self, addr: SocketAddr) {
        ...
        let mut records = route_table.all_records();
        let family_policy = InternalFamilyPolicy::new(is_ipv6);
        let now = StdInstant::now();
        ...
        record.failure_count = record.failure_count.saturating_add(1);
        record.last_queried_at = Some(now);
        let failure_limit = family_policy.soft_failure_limit(record);
        let failure_grace = family_policy.soft_failure_grace(record);
        let success_count = record.success_count;
        let lookup_success_count = record.lookup_success_count;
        let route_query_success_count = record.route_query_success_count;
        let failure_count = record.failure_count;
        let has_node_id = record.node_id.is_some();
        let retained = family_policy.retain_after_soft_failure(record);
        records.retain(|existing| family_policy.retain_after_soft_failure(existing));
        ...
        if internal_probe_enabled() {
            emit_internal_probe(format!(
                "event=soft_failure_detail family={} addr={} success_count={} lookup_success_count={} route_query_success_count={} failure_count={} failure_limit={} failure_grace={} retained={} total_before={} total_after={} removed={}",
                dht_family_label(is_ipv6),
                addr,
                success_count,
                lookup_success_count,
                route_query_success_count,
                failure_count,
                failure_limit,
                failure_grace,
                retained,
    ...
    fn snapshot_fast_frontier_state_for_family(
        ...
        let now = StdInstant::now();
        let family_nodes = route_table.snapshot_records();
        let live_nodes = family_nodes
            .into_iter()
            .filter(|record| family_policy.is_live_route_record(record, now))
            .collect::<Vec<_>>();
        let mut frontier = live_nodes
            .iter()
            .filter(|record| family_policy.has_recent_lookup_success(record, now))
            .cloned()
            .collect::<Vec<_>>();
        frontier.sort_by(|left, right| {
            compare_frontier_route_records(left, right, &target, is_ipv6, now)
        });
        ...
        if frontier.len() < INTERNAL_DHT_FAST_ACTIVE_FRONTIER_READY_FLOOR {
            let frontier_addrs = frontier
                ...
                .map(|record| record.addr)
                .collect::<HashSet<_>>();
            let mut route_query_supplemental = live_nodes
                .iter()
                .filter(|record| {
                    family_policy.has_recent_route_query_success(record, now)
                        && !frontier_addrs.contains(&record.addr)
                })
                .cloned()
                .collect::<Vec<_>>();
            route_query_supplemental.sort_by(|left, right| {
                compare_frontier_route_records(left, right, &target, is_ipv6, now)
            });
            frontier.extend(route_query_supplemental);
        }
        let recent_count = frontier.len().min(INTERNAL_DHT_FAST_ACTIVE_FRONTIER_LIMIT);
        if frontier.len() < INTERNAL_DHT_FAST_ACTIVE_FRONTIER_READY_FLOOR {
            let frontier_addrs = frontier
                .iter()
                .map(|record| record.addr)
                .collect::<HashSet<_>>();
            let mut supplemental = live_nodes
                .iter()
                .filter(|record| record.node_id.is_some() && !frontier_addrs.contains(&record.addr))
                .cloned()
                .collect::<Vec<_>>();
        ...
        if frontier.len() < INTERNAL_DHT_INITIAL_QUERY_FANOUT {
            ...
            let mut supplemental = live_nodes
                ...
                .collect::<Vec<_>>();
            supplemental.sort_by(|left, right| {
                compare_active_route_records(left, right, Some(&target), is_ipv6, now)
            });
            frontier.extend(supplemental);
        }
        frontier = family_policy.prioritize_lookup_proven_frontier(frontier);
        frontier = family_policy.finalize_fast_frontier(frontier);
        InternalFastFrontierSnapshot {
            nodes: frontier
                .into_iter()
    ...
    fn snapshot_for_family(&self, is_ipv6: bool, target: Option<[u8; 20]>) -> Vec<SocketAddr> {
        let family_policy = InternalFamilyPolicy::new(is_ipv6);
        let mut nodes = self.route_table(is_ipv6).snapshot_records();
        let now = StdInstant::now();
        nodes.sort_by(|left, right| {
            compare_active_route_records(left, right, target.as_ref(), is_ipv6, now)
        });
        nodes = family_policy.finalize_ranked_route_snapshot(nodes, target.is_some());
        nodes.into_iter().map(|record| record.addr).collect()
    }
    ...
    fn record_lookup_success(&mut self, addr: SocketAddr, node_id: Option<[u8; 20]>) {
            ...
            .find(|existing| existing.addr == addr)
        {
            ...
            record.last_queried_at = Some(now);
            record.last_success_at = Some(now);
            record.last_lookup_success_at = Some(now);
            ...
            candidate.last_queried_at = Some(now);
            candidate.last_success_at = Some(now);
            candidate.last_lookup_success_at = Some(now);
            ...
            if !should_admit_active_route_record(
                ...
                let promoted = route_table.insert_replacement_candidate(candidate);
                let after_total = route_table.total_count();
                let removed = before_total
                    .saturating_add(usize::from(promoted))
                    .saturating_sub(after_total);
                self.emit_probe_event(
                    if promoted {
                        "lookup_success_promoted"
                    } else {
    ...
    fn record_route_query_success(&mut self, addr: SocketAddr, node_id: Option<[u8; 20]>) {
        let is_ipv6 = addr.is_ipv6();
        let before_total = self.family_count(is_ipv6);
        let route_table = self.route_table_mut(is_ipv6);
        let active_snapshot = route_table.snapshot_records();
        ...
        {
            record.success_count = record.success_count.saturating_add(1);
            record.route_query_success_count = record.route_query_success_count.saturating_add(1);
            record.failure_count = 0;
            if let Some(node_id) = node_id {
                record.node_id = Some(node_id);
            }
            ...
            record.last_queried_at = Some(now);
            record.last_success_at = Some(now);
            record.last_route_query_success_at = Some(now);
            record.node_id.is_some()
        } else {
            let mut candidate = InternalPrototypeNodeRecord::new(addr);
            candidate.success_count = 1;
            candidate.route_query_success_count = 1;
            candidate.node_id = node_id;
            candidate.recency_epoch = recency_epoch;
            candidate.last_queried_at = Some(now);
            candidate.last_success_at = Some(now);
            candidate.last_route_query_success_at = Some(now);
            let has_node_id = candidate.node_id.is_some();
            if !should_admit_active_route_record(
                &active_snapshot,
                &candidate,
                ...
                self.emit_probe_event(
                    if promoted {
                        "route_query_success_promoted"
                    } else {
                        "route_query_success_rejected"
                    },
                    addr,
                    promoted,
                    removed,
    ...
    fn record_failure(&mut self, addr: SocketAddr) {
        ...
        if internal_probe_enabled() {
            emit_internal_probe(format!(
                "event=hard_failure_detail family={} addr={} retained={} total_before={} total_after={} removed={}",
                dht_family_label(is_ipv6),
                addr,
                retained,
                before_total,
    ...
    fn family_count(&self, is_ipv6: bool) -> usize {
        self.route_table(is_ipv6).total_count()
    }

    fn contains(&self, addr: SocketAddr) -> bool {
        self.route_table(addr.is_ipv6()).contains(addr)
    }

    fn snapshot_replacement_refresh_candidates_for_family(
        &self,
        is_ipv6: bool,
        target: Option<[u8; 20]>,
    ) -> Vec<SocketAddr> {
        self.route_table(is_ipv6)
            .snapshot_replacement_refresh_candidates(target)
    }

    fn snapshot_refresh_due_candidates_for_family(
        &self,
        is_ipv6: bool,
        target: Option<[u8; 20]>,
        limit: usize,
        ...
        self.route_table(is_ipv6)
            .snapshot_refresh_due_candidates(target, limit)
    }

    fn mark_queried(&mut self, addr: SocketAddr, when: StdInstant) -> bool {
        self.route_table_mut(addr.is_ipv6()).mark_queried(addr, when)
    }
}
...
struct InternalPrototypeNodeRecord {
    ...
    failure_count: u16,
    recency_epoch: u64,
    last_queried_at: Option<StdInstant>,
...
fn compare_recent_query_priority(
    left: &InternalPrototypeNodeRecord,
    right: &InternalPrototypeNodeRecord,
    family_policy: InternalFamilyPolicy,
    now: StdInstant,
) -> Ordering {
    family_policy
        .recent_peer_lookup_success_at(right, now)
        .cmp(&family_policy.recent_peer_lookup_success_at(left, now))
        .then_with(|| {
            family_policy
                .recent_lookup_success_at(right, now)
                .cmp(&family_policy.recent_lookup_success_at(left, now))
        })
        .then_with(|| {
            family_policy
                .recent_route_query_success_at(right, now)
                .cmp(&family_policy.recent_route_query_success_at(left, now))
        })
        .then_with(|| {
            family_policy
        ...
        .then_with(|| right.peer_lookup_success_count.cmp(&left.peer_lookup_success_count))
        .then_with(|| right.lookup_success_count.cmp(&left.lookup_success_count))
        .then_with(|| right.route_query_success_count.cmp(&left.route_query_success_count))
}

fn compare_refresh_due_records(
    left: &InternalPrototypeNodeRecord,
    right: &InternalPrototypeNodeRecord,
    is_ipv6: bool,
    now: StdInstant,
    ...
    let family_policy = InternalFamilyPolicy::new(is_ipv6);
    left.last_queried_at
        .cmp(&right.last_queried_at)
        .then_with(|| {
            family_policy
                .is_stale_route_record(right, now)
                .cmp(&family_policy.is_stale_route_record(left, now))
        })
        .then_with(|| left.failure_count.cmp(&right.failure_count))
        .then_with(|| right.last_success_at.cmp(&left.last_success_at))
...
fn compare_active_route_records(
    ...
    let live_order = family_policy
        .is_stale_route_record(left, now)
        .cmp(&family_policy.is_stale_route_record(right, now));
    if live_order != Ordering::Equal {
        return live_order;
    }

    let recent_query_order = compare_recent_query_priority(left, right, family_policy, now);
    if recent_query_order != Ordering::Equal {
        return recent_query_order;
    }
    ...
    let last_success_order = right.last_success_at.cmp(&left.last_success_at);
    if last_success_order != Ordering::Equal {
        return last_success_order;
    }

    let recency_order = right.recency_epoch.cmp(&left.recency_epoch);
    if recency_order != Ordering::Equal {
    ...
    let route_query_order = right
        .route_query_success_count
        .cmp(&left.route_query_success_count);
    if route_query_order != Ordering::Equal {
        return route_query_order;
    }
...
fn compare_active_route_retention_records_with_time(
    left: &InternalPrototypeNodeRecord,
    right: &InternalPrototypeNodeRecord,
    is_ipv6: bool,
    now: StdInstant,
    ...
    family_policy
        ...
        .then_with(|| left.failure_count.cmp(&right.failure_count))
        .then_with(|| compare_recent_query_priority(left, right, family_policy, now))
        .then_with(|| compare_active_route_retention_records(left, right))
        .then_with(|| right.last_success_at.cmp(&left.last_success_at))
        .then_with(|| right.last_route_query_success_at.cmp(&left.last_route_query_success_at))
        .then_with(|| right.last_lookup_success_at.cmp(&left.last_lookup_success_at))
        .then_with(|| {
            right
                .last_peer_lookup_success_at
                .cmp(&left.last_peer_lookup_success_at)
        })
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct InternalFamilyPolicy {
    is_ipv6: bool,
}

impl InternalFamilyPolicy {
    fn new(is_ipv6: bool) -> Self {
        Self { is_ipv6 }
    }
    ...
    fn is_stale_route_record(self, record: &InternalPrototypeNodeRecord, now: StdInstant) -> bool {
        record.last_success_at.is_none_or(|last_success_at| {
            now.saturating_duration_since(last_success_at) >= self.stale_route_age()
        })
    }

    fn is_live_route_record(self, record: &InternalPrototypeNodeRecord, now: StdInstant) -> bool {
        !self.is_stale_route_record(record, now)
    }

    fn recent_lookup_success_at(
        self,
        record: &InternalPrototypeNodeRecord,
        now: StdInstant,
    ) -> Option<StdInstant> {
        record
            .last_lookup_success_at
            .filter(|last_lookup_success_at| {
                now.saturating_duration_since(*last_lookup_success_at)
    ...
    fn recent_route_query_success_at(
        ...
        record
            .last_route_query_success_at
            .filter(|last_route_query_success_at| {
    ...
    fn has_recent_lookup_success(
        ...
        record.lookup_success_count > 0 && self.recent_lookup_success_at(record, now).is_some()
    }

    fn has_recent_route_query_success(
        self,
        record: &InternalPrototypeNodeRecord,
        now: StdInstant,
    ) -> bool {
        record.route_query_success_count > 0
            && self.recent_route_query_success_at(record, now).is_some()
    }

    fn is_fast_frontier_candidate(
        self,
        record: &InternalPrototypeNodeRecord,
        now: StdInstant,
    ) -> bool {
        self.is_live_route_record(record, now)
            && record.node_id.is_some()
            && (self.has_recent_lookup_success(record, now)
                || self.has_recent_route_query_success(record, now))
    }
    ...
    fn finalize_ranked_route_snapshot(
        self,
        nodes: Vec<InternalPrototypeNodeRecord>,
        target_present: bool,
    ) -> Vec<InternalPrototypeNodeRecord> {
        if !self.is_ipv6 && target_present {
            diversify_ipv4_route_records(nodes)
        } else {
            nodes
        }
    }

    fn prefers_fast_frontier_only_seeding(
        self,
        target_present: bool,
    ...
    fn max_frontier_candidates(self, purpose: &str) -> usize {
        if purpose == "lookup" {
            INTERNAL_DHT_LOOKUP_MAX_VISITS_PER_FAMILY
        } else {
            INTERNAL_DHT_BACKGROUND_MAX_VISITS_PER_FAMILY
        }
    }
    ...
    fn max_concurrent_queries(
        ...
        if before_first_batch {
            self.initial_query_fanout(purpose, fast_frontier_available)
    ...
    fn retain_after_soft_failure(self, record: &InternalPrototypeNodeRecord) -> bool {
        ...
        let failure_limit = self.soft_failure_limit(record);
        let failure_grace = self.soft_failure_grace(record);
        record.failure_count < failure_limit
...
fn is_bucket_eligible_route(record: &InternalPrototypeNodeRecord, _is_ipv6: bool) -> bool {
    record.node_id.is_some() && record.success_count > 0
}

fn trim_bucketed_routes(
    ...
    let Some(bucket_to_trim) = buckets
        ...
        .max_by(|(_, left_records), (_, right_records)| {
            left_records.len().cmp(&right_records.len()).then_with(|| {
                match (left_records.back(), right_records.back()) {
                    (Some(left_tail), Some(right_tail)) => {
                        compare_active_route_retention_records_with_time(
                            left_tail, right_tail, is_ipv6, now,
                        )
                    }
                    _ => Ordering::Equal,
                }
...
fn should_admit_active_route_record(
    ...
    let family_policy = InternalFamilyPolicy::new(is_ipv6);
    if existing.len() < family_policy.active_route_limit() {
        return true;
    }

    if family_policy.uses_bucketed_route_table()
        && candidate.lookup_success_count > 0
        && candidate.node_id.is_some()
    {
        if let Some(local_node_id) = local_node_id {
            ...
            return !bucket_counts.contains_key(&candidate_bucket)
    ...
    existing
        .iter()
        .max_by(|left, right| {
            compare_active_route_retention_records_with_time(left, right, is_ipv6, now)
        })
        .is_some_and(|worst| {
            if family_policy.is_stale_route_record(worst, now) {
                return !family_policy.is_stale_route_record(candidate, now)
...
impl InternalLookupFrontier {
    ...
    fn closest_unvisited_candidates(
        ...
        let mut candidates = self
            ...
            .collect::<Vec<_>>();
        candidates
            .sort_by(|left, right| compare_lookup_candidate_distance(left, right, &self.target));
        diversify_lookup_candidates(candidates, width)
    }
}

fn diversify_lookup_candidates(
    candidates: Vec<InternalLookupCandidate>,
    width: usize,
) -> Vec<InternalLookupCandidate> {
    if width == 0 {
    ...
    let mut seen_ipv4_subnets = HashSet::new();
    let mut seen_ipv6_prefixes = HashSet::new();
    let mut preferred = Vec::with_capacity(candidates.len());
    let mut remainder = Vec::new();

    for candidate in candidates {
        if !matches!(candidate.source, InternalQuerySource::Discovered) {
            preferred.push(candidate);
            continue;
        }

        match candidate.addr {
            SocketAddr::V4(_) => match ipv4_subnet_key(candidate.addr) {
                Some(subnet) if seen_ipv4_subnets.insert(subnet) => preferred.push(candidate),
                Some(_) => remainder.push(candidate),
                None => preferred.push(candidate),
            },
            SocketAddr::V6(_) => match ipv6_prefix_key(candidate.addr) {
                Some(prefix) if seen_ipv6_prefixes.insert(prefix) => preferred.push(candidate),
                Some(_) => remainder.push(candidate),
                None => preferred.push(candidate),
            },
        }
...
fn initial_family_query_fanout(
    ...
    InternalFamilyPolicy::new(is_ipv6).initial_query_fanout(purpose, fast_frontier_available)
}
...
fn family_max_concurrent_queries(
    ...
    InternalFamilyPolicy::new(is_ipv6).max_concurrent_queries(
        purpose,
        fast_frontier_available,
        before_first_batch,
    )
...
fn family_max_frontier_candidates(is_ipv6: bool, purpose: &str) -> usize {
    InternalFamilyPolicy::new(is_ipv6).max_frontier_candidates(purpose)
}
...
impl InternalLookupRoundPlanner {
    fn new(
        ...
        Self {
            ...
            max_visits: family_max_frontier_candidates(is_ipv6, purpose),
        }
    }
    ...
    fn followup_spawn_limit(self, join_set_len: usize) -> usize {
        if self.before_first_batch {
            self.max_concurrent_queries
                .saturating_sub(join_set_len)
                .min(INTERNAL_DHT_INITIAL_QUERY_FANOUT * 2)
        } else {
            self.max_concurrent_queries
...
impl InternalPrototypeClient {
    ...
    async fn recover_unhealthy_routes(&self) {
        tokio::join!(
            self.recover_family_routes(
                self.sockets.ipv4.as_ref(),
                &self.state.ipv4_bootstrap_nodes,
                false,
            ),
    ...
    async fn recover_family_routes(
        &self,
        socket: Option<&InternalPrototypeFamilySocket>,
        bootstrap_nodes: &HashSet<SocketAddr>,
        is_ipv6: bool,
        ...
        if internal_probe_enabled() {
            emit_internal_probe(format!(
                "event=route_recovery_start family={} fast_eligible_before={} positive_routes_before={} lookup_proven_before={} floor={}",
                dht_family_label(is_ipv6),
                before.fast_eligible,
                before.positive_routes,
                before.lookup_proven,
                INTERNAL_DHT_FAST_ACTIVE_FRONTIER_READY_FLOOR,
            ));
        }

        let mut discovered = self
            .warm_family_routes(Some(socket), bootstrap_nodes, is_ipv6)
            .await;
        if discovered.is_empty()
            && has_healthy_route_depth
            && before.lookup_proven == 0
            && before.fast_eligible == 0
            && before.positive_routes == 0
        {
            discovered.extend(
                self.recover_family_routes_from_refresh_candidates(socket, bootstrap_nodes, is_ipv6)
                    .await,
            );
        }
        let discovered_count = discovered.len();
        if !discovered.is_empty() {
            self.record_discovered_nodes(&discovered).await;
        }
        ...
        if internal_probe_enabled() {
            emit_internal_probe(format!(
                "event=route_recovery_done family={} discovered={} fast_eligible_after={} positive_routes_after={} lookup_proven_after={}",
                dht_family_label(is_ipv6),
                discovered_count,
                after.fast_eligible,
                after.positive_routes,
                after.lookup_proven,
            ));
        }
    }
    ...
    async fn refresh_family_routes(
        ...
        let discovered_before = self.discovered_nodes.lock().await.family_count(is_ipv6);
        let refill_floor = family_active_route_refill_floor(is_ipv6);
        let (before_summary, active_before, replacement_candidates, active_candidates) = {
            let active_routes = self.active_routes.lock().await;
            (
                active_routes.family_probe_summary(is_ipv6),
                active_routes.family_count(is_ipv6),
                active_routes.snapshot_replacement_refresh_candidates_for_family(
                    is_ipv6,
                    Some(self.node_id),
                ),
                active_routes
                    .snapshot_refresh_due_candidates_for_family(
                        is_ipv6,
                        Some(self.node_id),
                        INTERNAL_DHT_ROUTE_MAINTENANCE_LIMIT,
                    )
                    .into_iter()
                    .filter(|addr| !bootstrap_nodes.contains(addr))
                    .collect::<Vec<_>>(),
        ...
        let weak_frontier = before_summary.fast_eligible < INTERNAL_DHT_FAST_ACTIVE_FRONTIER_READY_FLOOR
            || before_summary.lookup_proven < INTERNAL_DHT_FAST_ACTIVE_FRONTIER_READY_FLOOR
            || before_summary.positive_routes < refill_floor;
        let query_refresh_budget = if weak_frontier { 4 } else { 2 };
        let ping_budget = INTERNAL_DHT_ROUTE_MAINTENANCE_LIMIT.saturating_sub(query_refresh_budget);
        let active_candidates = active_candidates
            .into_iter()
            .take(ping_budget)
            .collect::<Vec<_>>();
        let active_snapshot = active_candidates.iter().copied().collect::<HashSet<_>>();
        let mut candidates = active_candidates.clone();
        candidates.extend(
            self.discovered_nodes
                ...
                .into_iter()
                .filter(|addr| !bootstrap_nodes.contains(addr))
                .filter(|addr| !active_snapshot.contains(addr))
                .take(ping_budget.saturating_sub(candidates.len())),
        );
        let active_candidates = active_snapshot.len();
        let mut query_candidate_set = HashSet::new();
        let mut query_candidates = replacement_candidates
            .into_iter()
            .filter(|addr| !bootstrap_nodes.contains(addr))
            .filter(|addr| !active_snapshot.contains(addr))
            .filter(|addr| query_candidate_set.insert(*addr))
            .take(query_refresh_budget)
            .collect::<Vec<_>>();
        if query_candidates.len() < query_refresh_budget {
            query_candidates.extend(
                self.discovered_nodes
                    ...
                    .filter(|addr| !active_snapshot.contains(addr))
                    .filter(|addr| query_candidate_set.insert(*addr))
                    .take(query_refresh_budget.saturating_sub(query_candidates.len())),
            );
        }
        if weak_frontier && query_candidates.len() < query_refresh_budget {
            query_candidates.extend(
                self.discovered_nodes
                    .lock()
                    .await
                    .snapshot_recovery_candidates_for_family(is_ipv6, Some(self.node_id))
                    .into_iter()
                    .filter(|addr| !bootstrap_nodes.contains(addr))
                    .filter(|addr| !active_snapshot.contains(addr))
                    .filter(|addr| query_candidate_set.insert(*addr))
        ...
        let mut query_successes = 0usize;
        let mut query_failures = 0usize;
        let mut discovered = Vec::new();

        for candidate in candidates {
            self.mark_route_queried(candidate).await;
            if socket.ping(candidate, &self.node_id).await {
                self.record_route_refresh_success(candidate, None, "maintenance_refresh")
                    .await;
                successes += 1;
            } else {
                self.record_route_failure(candidate, "maintenance_refresh")
                    .await;
                failures += 1;
            }
        }
        let warm_targets = warm_lookup_targets(self.node_id);
        for (idx, candidate) in query_candidates.iter().copied().enumerate() {
            self.mark_route_queried(candidate).await;
            let lookup_target = warm_targets[idx % warm_targets.len()];
            let Some(response) = socket
                .find_node(candidate, &self.node_id, &lookup_target)
                .await
            else {
                self.record_route_failure(candidate, "maintenance_refresh_query")
                    .await;
                query_failures += 1;
                continue;
            };
            self.record_route_query_success(candidate, response.node_id())
                .await;
            query_successes += 1;
        ...
        if internal_probe_enabled() {
            let discovered_after = self.discovered_nodes.lock().await.family_count(is_ipv6);
            let active_after = self.active_routes.lock().await.family_count(is_ipv6);
            emit_internal_probe(format!(
                "event=maintenance_refresh_summary family={} candidates={} active_candidates={} successes={} failures={} query_candidates={} query_successes={} query_failures={} query_discovered={} weak_frontier={} discovered_before={} discovered_after={} active_before={} active_after={} limit={}",
                dht_family_label(is_ipv6),
                successes + failures,
                active_candidates,
                successes,
    ...
    async fn warm_family_routes(
        ...
        loop {
            ...
            let Some(response) = response else {
                self.record_route_failure(candidate, "route_warm").await;
                if join_set.is_empty()
            ...
            if pending_discovered.len() > 1 {
                ...
                reordered.sort_by(|left, right| {
                    compare_compact_node_distance(left, right, &self.node_id)
                });
                pending_discovered = reordered.into();
            }

            if join_set.is_empty() && pending_discovered.is_empty() && pending_bootstrap.is_empty()
            {
                break;
            }
        }
    ...
    async fn recover_family_routes_from_refresh_candidates(
        &self,
        socket: &InternalPrototypeFamilySocket,
        bootstrap_nodes: &HashSet<SocketAddr>,
        is_ipv6: bool,
        ...
        let mut discovered = Vec::new();
        let refresh_limit = INTERNAL_DHT_ROUTE_MAINTENANCE_LIMIT;
        let mut queued = HashSet::new();
        let mut candidates = self
            .active_routes
            .lock()
            .await
            .snapshot_replacement_refresh_candidates_for_family(is_ipv6, Some(self.node_id))
            .into_iter()
            .filter(|addr| !bootstrap_nodes.contains(addr))
            .collect::<Vec<_>>();
        let replacement_candidate_count = candidates.len();
        for candidate in &candidates {
            queued.insert(*candidate);
        }
        if candidates.len() < refresh_limit {
            candidates.extend(
                self.discovered_nodes
                    .lock()
                    .await
                    ...
                    .into_iter()
                    .filter(|addr| !bootstrap_nodes.contains(addr))
                    .filter(|addr| queued.insert(*addr))
                    .take(refresh_limit.saturating_sub(candidates.len())),
            );
        }
        if internal_probe_enabled() {
            emit_internal_probe(format!(
                "event=route_recovery_refresh_candidates family={} candidates={} replacement_candidates={} cached_candidates={}",
                dht_family_label(is_ipv6),
                candidates.len(),
                replacement_candidate_count,
                candidates.len().saturating_sub(replacement_candidate_count),
            ));
        }
        let warm_targets = warm_lookup_targets(self.node_id);
        ...
        for (idx, candidate) in candidates.into_iter().enumerate() {
            ...
            let Some(response) = socket
                ...
                self.record_route_failure(candidate, "route_recovery_cached")
                    .await;
                continue;
            };
            self.record_route_query_success(candidate, response.node_id())
            ...
            if discovered.len() >= INTERNAL_DHT_ROUTE_WARM_MAX_VISITS {
                break;
            }
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn internal_prototype_maintenance_tick_pings_active_routes_even_if_not_discovered() {
        ...
        assert!(
            ...
            "maintenance should refresh active routes directly, even when they are not in the discovered cache"
        );
    ...
    #[tokio::test]
    async fn internal_prototype_maintenance_tick_queries_replacement_candidates_when_ipv4_frontier_is_weak(
    ) {
        let (replacement_tx, mut replacement_rx) = mpsc::unbounded_channel();
        let (replacement_addr, replacement_task) = spawn_observing_test_krpc_server(
            "127.0.0.1:0".parse().expect("replacement bind addr"),
            TestKrpcReply::default(),
            Some(replacement_tx),
        )
        .await;
...

        {
            ...
            for idx in 0..INTERNAL_DHT_IPV4_ACTIVE_ROUTE_REFILL_FLOOR {
                let addr = SocketAddr::new(
                    IpAddr::V4(Ipv4Addr::new(127, 5, 0, (idx % 250 + 1) as u8)),
                    49600 + u16::try_from(idx).expect("port fits"),
                );
            ...
            let now = StdInstant::now();
            {
                ...
                for record in &mut records {
                    record.last_success_at =
                        Some(now - (INTERNAL_DHT_IPV4_STALE_ROUTE_AGE + Duration::from_secs(1)));
                    record.last_lookup_success_at = None;
                    record.last_peer_lookup_success_at = None;
                    record.last_route_query_success_at = None;
                }
                route_table.replace_records(records);
            }

            let route_table = active_routes.route_table_mut(false);
            let mut replacement = InternalPrototypeNodeRecord::new(replacement_addr);
            replacement.node_id = Some(test_bucketed_node_id_for_local(local_node_id, 80, 99));
            replacement.success_count = 1;
            replacement.route_query_success_count = 1;
            replacement.recency_epoch = route_table.allocate_recency_epoch();
            replacement.last_success_at = Some(now);
            replacement.last_route_query_success_at = Some(now);
            route_table.insert_replacement_candidate(replacement);
        }

        client.force_maintenance_due(true, false).await;
        client.maintenance_tick().await;

        let observations = drain_observations(&mut replacement_rx).await;
        assert!(
            observations
                .iter()
                .any(|observation| {
                    matches!(
                        observation,
                        TestKrpcObservation::FindNode | TestKrpcObservation::Ping
                    )
                }),
            "maintenance should either query or directly promote vetted replacement candidates when the IPv4 frontier has gone stale"
        );
    ...
    #[tokio::test]
    async fn recovery_promotes_replacement_candidates_when_full_ipv4_table_goes_unproductive() {
        let (replacement_addr, replacement_task) = spawn_observing_test_krpc_server(
            "127.0.0.1:0".parse().expect("replacement bind addr"),
            TestKrpcReply::default(),
            None,
        ...
        .await;
        let (bootstrap_addr, bootstrap_task) = spawn_blackhole_test_krpc_server(
            "127.0.0.1:0".parse().expect("bootstrap bind addr"),
            None,
        )
        .await;
...

        {
            ...
            for idx in 0..INTERNAL_DHT_IPV4_ACTIVE_ROUTE_REFILL_FLOOR {
                let addr = SocketAddr::new(
                    IpAddr::V4(Ipv4Addr::new(127, 3, 0, (idx % 250 + 1) as u8)),
                    49200 + u16::try_from(idx).expect("port fits"),
                );
                let bucket_key = ((idx % 8) as u8 + 1) * 8;
                active_routes.record_success(
                    addr,
            ...
            let route_table = active_routes.route_table_mut(false);
            let mut replacement = InternalPrototypeNodeRecord::new(replacement_addr);
            replacement.node_id = Some(test_bucketed_node_id_for_local(local_node_id, 72, 99));
            replacement.success_count = 1;
            replacement.route_query_success_count = 1;
            replacement.recency_epoch = route_table.allocate_recency_epoch();
            replacement.last_success_at = Some(now);
        ...
        client
            .recover_family_routes(
                client.sockets.ipv4.as_ref(),
        ...
        let active_routes = client.active_routes.lock().await;
        assert!(
            active_routes.contains(replacement_addr),
            "recovery should promote responsive replacement routes back into the live IPv4 table when the current table is full but dead"
        );
    ...
    #[tokio::test]
    async fn recovery_promotes_cached_candidates_when_full_ipv4_table_goes_unproductive() {
        let (cached_addr, cached_task) = spawn_observing_test_krpc_server(
            "127.0.0.1:0".parse().expect("cached bind addr"),
            TestKrpcReply::default(),
            None,
        )
        .await;
...

        {
            ...
            for idx in 0..INTERNAL_DHT_IPV4_ACTIVE_ROUTE_REFILL_FLOOR {
                let addr = SocketAddr::new(
                    IpAddr::V4(Ipv4Addr::new(127, 4, 0, (idx % 250 + 1) as u8)),
                    49400 + u16::try_from(idx).expect("port fits"),
                );
                let bucket_key = ((idx % 8) as u8 + 1) * 8;
                active_routes.record_success(
                    addr,
        ...
        let active_routes = client.active_routes.lock().await;
        assert!(
            active_routes.contains(cached_addr),
            "recovery should promote responsive cached routes back into the live IPv4 table when the current table is full but dead"
        );
    ...
    async fn internal_prototype_maintenance_tick_evicts_failed_discovered_routes() {
        ...
        assert!(
            blackhole_observations
                .iter()
                .any(|observation| matches!(observation, TestKrpcObservation::Ping)),
            "maintenance should still probe failed discovered routes directly"
        );
    ...
    async fn seed_family_nodes_keeps_bootstrap_out_of_initial_wave_when_frontier_is_deep() {
        ...
        client.record_discovered_nodes(&cached_nodes).await;
        {
            ...
            for idx in 0..INTERNAL_DHT_INITIAL_QUERY_FANOUT {
                ...
                active_routes
                    .record_lookup_success(addr, Some(test_node_id((idx as u8).wrapping_add(80))));
            }
        }
    ...
    async fn seed_family_nodes_excludes_broad_cached_routes_for_mature_ipv4_fast_frontier() {
        ...
        let discovered_only_addr: SocketAddr = "127.0.0.1:49085".parse().expect("discovered addr");
        let (client, warning) = InternalPrototypeClient::bind(0, &[])
            .await
    ...
    #[test]
    fn discovered_round_diversifies_ipv4_subnets() {
        let candidates = vec![
            InternalLookupCandidate {
                addr: "203.0.113.1:40120".parse().expect("candidate addr"),
                node_id: Some(test_node_id(1)),
                source: InternalQuerySource::Discovered,
                insertion_order: 0,
            },
            InternalLookupCandidate {
                addr: "203.0.113.2:40121".parse().expect("candidate addr"),
                node_id: Some(test_node_id(2)),
                source: InternalQuerySource::Discovered,
                insertion_order: 1,
            },
            InternalLookupCandidate {
                addr: "198.51.100.1:40122".parse().expect("candidate addr"),
                node_id: Some(test_node_id(3)),
                source: InternalQuerySource::Discovered,
                insertion_order: 2,
            },
        ];

        let ordered = diversify_lookup_candidates(candidates, 3)
            .into_iter()
        ...
        assert_eq!(
            ordered,
            vec![
                "203.0.113.1:40120".parse().expect("candidate addr"),
                "198.51.100.1:40122".parse().expect("candidate addr"),
                "203.0.113.2:40121".parse().expect("candidate addr"),
            ]
        );
    }

    #[test]
    fn discovered_round_diversifies_ipv6_prefixes() {
        let candidates = vec![
            InternalLookupCandidate {
                addr: "[2001:db8:1::1]:40130".parse().expect("candidate addr"),
                node_id: Some(test_node_id(1)),
                source: InternalQuerySource::Discovered,
                insertion_order: 0,
            },
            InternalLookupCandidate {
                addr: "[2001:db8:1::2]:40131".parse().expect("candidate addr"),
                node_id: Some(test_node_id(2)),
                source: InternalQuerySource::Discovered,
                insertion_order: 1,
            },
            InternalLookupCandidate {
                addr: "[2001:db8:2::1]:40132".parse().expect("candidate addr"),
                node_id: Some(test_node_id(3)),
                source: InternalQuerySource::Discovered,
                insertion_order: 2,
            },
        ...
        assert_eq!(
            ordered,
            vec![
                "[2001:db8:1::1]:40130".parse().expect("candidate addr"),
                "[2001:db8:2::1]:40132".parse().expect("candidate addr"),
                "[2001:db8:1::2]:40131".parse().expect("candidate addr"),
            ]
        );
    }
    ...
    fn active_route_frontier_prefers_route_query_proven_routes_before_route_only_routes() {
        ...
        routes.record_route_query_success(route_query_addr, Some(test_node_id(7)));
        routes.record_success(route_only_addr, Some(test_node_id(1)));
        routes.record_success(route_only_addr, Some(test_node_id(1)));
        routes.record_success(route_only_addr, Some(test_node_id(1)));
        ...
        assert_eq!(snapshot.nodes, vec![route_query_addr, route_only_addr]);
    }
    ...
    #[test]
    fn both_families_share_frontier_candidate_limit_per_purpose() {
        assert_eq!(
            family_max_frontier_candidates(false, "lookup"),
            INTERNAL_DHT_LOOKUP_MAX_VISITS_PER_FAMILY
        );
        assert_eq!(
            family_max_frontier_candidates(true, "lookup"),
            INTERNAL_DHT_LOOKUP_MAX_VISITS_PER_FAMILY
        );
        assert_eq!(
            family_max_frontier_candidates(false, "announce_token_refresh"),
            INTERNAL_DHT_BACKGROUND_MAX_VISITS_PER_FAMILY
        );
        assert_eq!(
            family_max_frontier_candidates(true, "announce_token_refresh"),
            INTERNAL_DHT_BACKGROUND_MAX_VISITS_PER_FAMILY
        );
    }
    ...
    fn lookup_round_planner_preserves_current_before_and_after_batch_behavior() {
        ...
        assert_eq!(
            before_first_batch.idle_spawn_limit(),
            2
        );
        assert_eq!(
            before_first_batch.followup_spawn_limit(3),
            INTERNAL_DHT_INITIAL_QUERY_FANOUT * 2
        );
    ...
    #[test]
    fn active_route_hard_failure_does_not_sweep_other_proven_routes() {
        let failed_addr = "127.0.0.1:40156".parse().expect("failed route addr");
        let surviving_a = "127.0.0.1:40157".parse().expect("surviving route a");
        let surviving_b = "127.0.0.1:40158".parse().expect("surviving route b");
        let mut routes = InternalPrototypeActiveRoutes::default();

        for _ in 0..4 {
            routes.record_lookup_success(failed_addr, Some(test_node_id(21)));
            routes.record_lookup_success(surviving_a, Some(test_node_id(22)));
            routes.record_lookup_success(surviving_b, Some(test_node_id(23)));
        }
        ...
        assert!(ordered.contains(&surviving_a));
        assert!(ordered.contains(&surviving_b));
    }
    ...
    #[test]
    fn route_success_resets_failure_debt() {
        let addr = "127.0.0.1:40155".parse().expect("route success reset addr");
        let mut routes = InternalPrototypeActiveRoutes::default();
        routes.record_lookup_success(addr, Some(test_node_id(15)));
    ...
    #[test]
    fn lookup_proven_ipv4_routes_survive_more_soft_failures() {
        let addr = "127.0.0.1:40153"
            .parse()
            .expect("lookup-proven soft failure addr");
        let mut routes = InternalPrototypeActiveRoutes::default();
        for _ in 0..INTERNAL_DHT_IPV4_PROVEN_ROUTE_SOFT_FAILURE_LIMIT {
            routes.record_lookup_success(addr, Some(test_node_id(13)));
        }
    ...
    #[test]
    fn lookup_proven_ipv4_routes_remain_in_fast_frontier_after_soft_failures() {
        let addr = "127.0.0.1:40157"
            .parse()
            .expect("lookup-proven frontier addr");
        let mut routes = InternalPrototypeActiveRoutes::default();
        routes.record_lookup_success(addr, Some(test_node_id(16)));
        routes.record_lookup_success(addr, Some(test_node_id(16)));
    ...
    #[test]
    fn stale_ipv4_routes_drop_out_of_fast_frontier() {
        let stale_addr = "127.0.0.1:40158".parse().expect("stale route addr");
        let fresh_addr = "127.0.0.1:40159".parse().expect("fresh route addr");
        let mut routes = InternalPrototypeActiveRoutes::default();

        routes.record_lookup_success(stale_addr, Some(test_node_id(17)));
        routes.record_lookup_success(stale_addr, Some(test_node_id(17)));
        routes.record_lookup_success(fresh_addr, Some(test_node_id(18)));
        routes.record_lookup_success(fresh_addr, Some(test_node_id(18)));

        let stale_cutoff =
            StdInstant::now() - (INTERNAL_DHT_IPV4_STALE_ROUTE_AGE + Duration::from_secs(1));
        let route_table = &mut routes.ipv4;
        let mut records = route_table.snapshot_records();
        let stale_record = records
            .iter_mut()
            .find(|record| record.addr == stale_addr)
            .expect("stale record");
        stale_record.last_success_at = Some(stale_cutoff);
        route_table.replace_records(records);

        let snapshot = routes.snapshot_fast_frontier_state_for_family(false, Some([0u8; 20]));

        assert_eq!(snapshot.lookup_proven_count, 1);
        assert_eq!(snapshot.recent_count, 1);
        assert_eq!(snapshot.nodes.first().copied(), Some(fresh_addr));
    }

    #[test]
    fn refresh_due_candidates_prefer_unqueried_then_oldest_routes() {
        let never_queried_addr = "127.0.0.1:40161".parse().expect("never queried addr");
        let oldest_queried_addr = "127.0.0.1:40162".parse().expect("oldest queried addr");
        let newest_queried_addr = "127.0.0.1:40163".parse().expect("newest queried addr");
        let mut routes = InternalPrototypeActiveRoutes::default();

        routes.record_success(never_queried_addr, Some(test_bucketed_node_id(24, 1)));
        routes.record_success(oldest_queried_addr, Some(test_bucketed_node_id(32, 1)));
        routes.record_success(newest_queried_addr, Some(test_bucketed_node_id(40, 1)));
        ...
        records
            .iter_mut()
            .find(|record| record.addr == never_queried_addr)
            .expect("never queried record")
            .last_queried_at = None;
        records
            .iter_mut()
            .find(|record| record.addr == oldest_queried_addr)
            .expect("oldest queried record")
            .last_queried_at = Some(now - Duration::from_secs(90));
        records
            .iter_mut()
            .find(|record| record.addr == newest_queried_addr)
            .expect("newest queried record")
            .last_queried_at = Some(now - Duration::from_secs(15));
        route_table.replace_records(records);

        let ordered = routes.snapshot_refresh_due_candidates_for_family(false, None, 3);

        assert_eq!(
            ordered,
            vec![never_queried_addr, oldest_queried_addr, newest_queried_addr]
        );
    }

    #[test]
    fn historical_ipv4_query_success_drops_out_of_fast_frontier_when_only_maintenance_is_recent() {
        let historical_addr = "127.0.0.2:40158".parse().expect("historical route addr");
        let fresh_addr = "127.0.0.3:40159".parse().expect("fresh route addr");
        let mut routes = InternalPrototypeActiveRoutes::default();

        routes.record_lookup_success(historical_addr, Some(test_node_id(31)));
        routes.record_lookup_success(historical_addr, Some(test_node_id(31)));
        routes.record_lookup_success(fresh_addr, Some(test_node_id(32)));
        routes.record_lookup_success(fresh_addr, Some(test_node_id(32)));

        let recent_now = StdInstant::now();
        let stale_query_cutoff =
            recent_now - (INTERNAL_DHT_IPV4_RECENT_QUERY_SUCCESS_AGE + Duration::from_secs(1));
        let route_table = &mut routes.ipv4;
        let mut records = route_table.snapshot_records();
        let historical_record = records
            .iter_mut()
            .find(|record| record.addr == historical_addr)
            .expect("historical record");
        historical_record.last_lookup_success_at = Some(stale_query_cutoff);
        historical_record.last_success_at = Some(recent_now);
        route_table.replace_records(records);
    ...
    #[test]
    fn targeted_ipv4_seed_order_prefers_recent_query_success_over_historical_hits() {
        let historical_addr = "127.0.1.10:40160".parse().expect("historical route addr");
        let recent_addr = "127.0.2.10:40161".parse().expect("recent route addr");
        let historical_node_id = test_node_id(41);
        let recent_node_id = test_node_id(99);
        let mut routes = InternalPrototypeActiveRoutes::default();

        routes.record_lookup_success(historical_addr, Some(historical_node_id));
        routes.record_lookup_success(historical_addr, Some(historical_node_id));
        routes.record_lookup_success(recent_addr, Some(recent_node_id));
        routes.record_lookup_success(recent_addr, Some(recent_node_id));
        ...
        let ordered = routes.snapshot_for_family(false, Some(historical_node_id));

        assert_eq!(ordered.first().copied(), Some(recent_addr));
    }

    #[test]
    fn stale_ipv4_routes_are_replaceable_when_table_is_full() {
        let mut routes = InternalPrototypeActiveRoutes::default();
        let existing_addrs = (0..INTERNAL_DHT_IPV4_ACTIVE_ROUTE_LIMIT)
            .map(|idx| format!("127.0.9.{}:{}", (idx % 250) + 1, 52000 + idx))
        ...
        let stale_addr = existing_addrs[0];
        let stale_cutoff =
            StdInstant::now() - (INTERNAL_DHT_IPV4_STALE_ROUTE_AGE + Duration::from_secs(1));
        let route_table = &mut routes.ipv4;
        let mut records = route_table.snapshot_records();
        ...
        let candidate_addr = "127.0.10.1:53001".parse().expect("candidate addr");
        ...
        assert!(!ordered.contains(&stale_addr));
    }
    ...
    async fn route_refresh_success_does_not_promote_new_active_route() {
        ...
        client
            .record_route_refresh_success(addr, None, "test")
            .await;
    ...
    #[tokio::test]
    async fn route_refresh_success_can_reinforce_healthy_ipv4_pool() {
        ...
        client
            .record_route_refresh_success(refill_addr, None, "test")
            .await;
    ...
    async fn lookup_failures_do_not_immediately_demote_active_routes() {
        ...
        client
            .record_lookup_success(addr, Some(test_node_id(15)))
            .await;
