...
BEP 9 metadata parsing, bounded requests, replies, and verified-availability notifications are shared by TCP/uTP and WebRTC sessions. RTC identity verification remains transport-specific. All sessions reject metadata sizes outside 1 byte through 16 MiB. TorrentManager serves the accepted info dictionary and advertises its exact byte length; state still accepts metadata after info-hash verification.
