...
Native builds include WebTorrent by default (`cargo run`); there is no runtime enable switch. Older configurations containing `webtorrent.enabled` still load, but the obsolete field is ignored. Optional `[[webtorrent.ice_servers]]` entries accept `urls`, `username`, and `credential`. No public tracker or ICE server is injected. Initially supported: native public v1/hybrid torrents, including metadata bootstrap, unrestricted `Any` binding, system DNS, both IP families, and no peer-policy restrictions. Other combinations fail closed; WebTorrent does not bypass those restrictions. Unknown metadata is eligible for discovery and stops RTC execution if later verified private or unsupported.

Signaling and negotiation failures remain isolated from ordinary peer transports and use the existing tracker retry and connection cleanup paths. Native WebRTC eligibility reads the resolved policy from the active network lease, so live routing changes do not depend on a manager settings refresh. Tracker and peer startup retain the activation snapshot they checked; invalidation cancels that scope instead of panicking or switching an admitted peer to another generation. The simulated Wasm demo and storage harness explicitly retain their original DHT/PEX feature set; the full browser client selects WebTorrent.

Limits: four signaling services per torrent, four setup jobs/pending offers per tracker, two offers per authorized announce, 256 KiB tracker frames, 64 KiB SDP, 16 MiB metadata acquisition, 16 queued metadata requests per peer, a 256 KiB stream window, and 16 KiB outgoing DataChannel chunks. Setup/response and socket-write deadlines return failures to the existing state backoff path. Physical connection permits survive canceled negotiations until library close completes. Normal session shutdown awaits close; cancellation cleanup is asynchronous, so runtime teardown still requires the host to drain its tasks.

Run the deterministic native contracts with `cargo test --lib --locked`. Install the browser test dependencies with `cd web && npm ci`, then run `cargo test --lib --locked rtc_contracts -- --ignored --nocapture` from the repository root. The two Chromium contracts use generated content and a local signaling relay, exercise real manager magnet download and verified seeding, and check the bytes in both directions. The magnet peer also requests verified metadata back on the same connection. These establish native-to-browser interoperability; public trackers, NAT/TURN deployment, other browser engines, and a full Wasm torrent manager remain separate qualification gates.
...
### 12.9 Integration cleanup boundaries

BEP 9 metadata parsing, bounded requests, replies, and verified-availability notifications are shared by TCP/uTP and WebRTC sessions. RTC identity verification remains transport-specific. Sessions acquiring missing metadata reject advertised sizes outside 1 byte through 16 MiB. Sessions with metadata already available retain payload connections regardless of the advertised metadata size. TorrentManager serves the accepted info dictionary and advertises its exact byte length; state still accepts metadata after info-hash verification.

Physical I/O retains immutable `Arc<MultiFileInfo>` snapshots and shared owned write buffers. TorrentManager invalidates its execution snapshot after metadata/configuration actions; export authorization always reads current state before obtaining a snapshot. OPFS compares physical layouts when the snapshot identity changes, while ordinary operations reuse the accepted snapshot. Backend admission, cancellation-held permits, write completion, and close/remove barriers remain intact.

The browser session owns accepted command transitions, catalog rows, stopped-manager recovery, and restart/removal decisions. The engine host executes the resulting physical work: constructing managers, retaining task/OPFS handles, persisting the catalog, and deleting stopped payloads. Demo composition remains separate.

Native tracker operations live in `torrent_manager/tracker_execution.rs` as TorrentManager methods. The shared manager retains scheduling, network-generation reconciliation, state actions, and terminal storage cleanup. RTC negotiation helpers separate offer preparation, answering, connection completion, and bounded socket writes without adding another scheduler.
