...
use super::krpc::{
    ...
    KrpcIncomingQuery, KrpcPingArgs, KrpcQueryEnvelope, KrpcQueryKind, KrpcResponseBody,
    KrpcResponseEnvelope,
};
...
impl TransportActor {
        ...
        A: Serialize,
    {
        let (transaction_id, response_rx) =
            self.send_query_deferred(target, query, args).await?;
    ...
    fn register_inflight_query(
        ...
        loop {
            let transaction_id =
                TransactionId::from(self.inner.next_transaction_id.fetch_add(
                    1,
...
fn trace_get_peers_payload(node_id: NodeId, info_hash: InfoHash, args: &KrpcGetPeersArgs) {
    ...
    let enabled =
        *TRACE_PAYLOAD.get_or_init(|| std::env::var_os("SUPERSEEDR_INTERNAL_TRACE_PAYLOAD").is_some());
    if !enabled {
        return;
    }
    ...
    let synthetic = KrpcQueryEnvelope::new(TransactionId::from([0, 0, 0, 0]), KrpcQueryKind::GetPeers, args.clone());
