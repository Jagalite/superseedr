...
pub(crate) async fn execute_native_confirm_decision(
    app: &mut App,
    decision: crate::tui::interaction_effects::ConfirmDecision,
...
pub(crate) async fn handle_event_at(event: Event, app: &mut App, now: Instant) {
    let pending_text = crate::tui::reducer::pending_paste_text_before_event(&event, &app.app_state)
        .map(str::to_owned);
    let pending_is_paste = pending_text
        .as_deref()
        .is_some_and(|text| app.accepts_pasted_text(text));
    let translated =
        crate::tui::reducer::translate_event(event, &mut app.app_state, now, pending_is_paste);
    reduce_and_execute_events(app, translated).await;
}

pub(crate) async fn flush_pending_paste_burst_at(app: &mut App, now: Instant) {
    let pending_text = crate::tui::reducer::due_paste_text(&app.app_state, now).map(str::to_owned);
    let pending_is_paste = pending_text
        .as_deref()
        .is_some_and(|text| app.accepts_pasted_text(text));
    let translated =
        crate::tui::reducer::flush_due_events(&mut app.app_state, now, pending_is_paste);
    reduce_and_execute_events(app, translated).await;
}
...
async fn reduce_and_execute_events(app: &mut App, events: Vec<Event>) {
    ...
    for event in events {
        ...
        let effects = crate::tui::reducer::reduce_event(
            event,
            &mut app.app_state,
            &settings,
            shared_follower,
